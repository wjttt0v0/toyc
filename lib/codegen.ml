  ...
  | None ->
      ...
      | EBinOp (op, e1, e2) ->
          if op = And then
            ...
            let (e1_code, _) = gen_expr env regs e1 in
            let (e2_code, regs_after_e2) = gen_expr env regs e2 in
            (e1_code @ [Printf.sprintf "  beqz %s, %s" target_reg false_label] @ e2_code
             @ [Printf.sprintf "  snez %s, %s" target_reg target_reg;
                ...
                Printf.sprintf "%s:" end_label], regs_after_e2)
          else if op = Or then
            ...
            let (e2_code, regs_after_e2) = gen_expr env regs_after_e1 e2 in
            let r2 = List.hd regs_after_e1 in
            let op_code = match op with
...
and gen_stmt env stmt : string list =
  ...
  | SIf (cond, then_stmt, else_opt) ->
      ...
      let cond_gen_code =
        match cond with
        | EBinOp (op, e1, e2) ->
            let (e1_code, regs1) = gen_expr env temp_regs e1 in
            let (e2_code, _) = gen_expr env regs1 e2 in
            let r1 = List.hd temp_regs in
            let r2 = List.hd regs1 in
            let branch_op = match op with
              | Eq  -> "bne" | Neq -> "beq" | Lt  -> "bge" | Ge  -> "blt"
              | Gt  -> "ble" | Le  -> "bgt" | _ -> ""
            in
            if branch_op <> "" then
              e1_code @ e2_code @ [Printf.sprintf "  %s %s, %s, %s" branch_op r1 r2 else_label]
            else 
              let (full_expr_code, _) = gen_expr env temp_regs cond in
              full_expr_code @ [Printf.sprintf "  beqz %s, %s" (List.hd temp_regs) else_label]
        | _ ->
            let (e_code, _) = gen_expr env temp_regs cond in
            e_code @ [Printf.sprintf "  beqz %s, %s" (List.hd temp_regs) else_label]
      in
      
      let then_code = gen_stmt env then_stmt in
      let else_code = match else_opt with
        | Some s -> gen_stmt env s
        | None -> []
      in
      
      cond_gen_code @ then_code @
      (if else_code <> [] then [Printf.sprintf "  j %s" end_label] else []) @
      [Printf.sprintf "%s:" else_label] @
      else_code @
      (if else_code <> [] then [Printf.sprintf "%s:" end_label] else [])
  ...
  | SWhile (cond, body) ->
      ...
      @ cond_code
      @ [Printf.sprintf "  beqz %s, %s" (List.hd temp_regs) end_label]
...
let gen_func_def f : string list =
  ...
  let stack_size = 8 (* ra & old fp *) + (num_locals * 4) in
  ...
  let (initial_env, _) =
    ...
    in
    build_env_for_decls { var_map = []; current_offset = -8; exit_label; break_label=None; continue_label=None } (-8) (match f.body with SBlock stmts -> stmts | s -> [s])
  in
  let body_code = gen_stmt initial_env f.body in
